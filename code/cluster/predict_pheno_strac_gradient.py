...
parser.add_argument("-grad", help="gradient file", dest="grad_file", default=None)
parser.add_argument("-direction", help="direction of swapping str for ac", dest="direction", default='strac')
...
# load gradient
g = 0
gradient = np.loadtxt(grad_file)[:,g]
sort_idx = np.argsort(gradient)

# inputs
X_full = pd.read_csv(X_file)
X_full.set_index(['bblid', 'scanid'], inplace = True)
X_str = X_full.filter(regex = 'str').copy()
X_str = X_str.iloc[:,sort_idx]
X_ac = X_full.filter(regex = 'ac').copy()
X_ac = X_ac.iloc[:,sort_idx]
if direction == 'strac':
    X = pd.concat((X_str.iloc[:,:seed+1], X_ac.iloc[:,seed+1:]), axis = 1)
elif direction == 'acstr':
    X = pd.concat((X_ac.iloc[:,:seed+1], X_str.iloc[:,seed+1:]), axis = 1)
...
# outdir
outdir = os.path.join(outroot, score, 'seed_' + str(seed), alg + '_' + pheno)
